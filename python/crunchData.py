...
def makeDistMatrix(lst, metric):
    """
    Given a list, find distance between all pairs in that list. Note that this
    is quite general, and will work for any list and metric, so long as the
    metric is defined for that list
    ...
    distanceMatrix = [ {'pair': p, 'dist': metric(lstPair(p)) } for p in allPairs]
